...
if (Key.isDown(Key.W)) {
	if (testCol(camera.position.x - (Math.sin(this.camera.rotation.y) / fps), camera.position.y-1, camera.position.z - (Math.cos(this.camera.rotation.y) / fps))) {
		this.camera.position.x -= 1 * Math.sin(this.camera.rotation.y) / fps;
		this.camera.position.z -= 1 * Math.cos(this.camera.rotation.y) / fps;
	}
}
if (Key.isDown(Key.A)) {
	if (testCol(camera.position.x - (Math.sin(this.camera.rotation.y + Math.PI/2) / fps), camera.position.y-1, camera.position.z - (Math.cos(this.camera.rotation.y + Math.PI/2) / fps))) {
...
if (Key.isDown(Key.R)) {
    ...
    for (removecounter = 0; removecounter <= bullets.length; removecounter++) {
        scene.remove(bullets[removecounter]);
    }
    bullrot = [];
	bullets = [];
	modes = [];
	bullarrindexnum = 0;
	for (vr = 0; vr < voxels.length; vr++) {
		voxels[vr].material = new THREE.MeshBasicMaterial({color: 0x888888});
	}
}
...
if (Key.isDown(Key.E)) {
    if (bullbool == true) {
    	if (mode == 'block') {
        	...
        	bullrot[bullarrindexnum] = camera.rotation;
...
for (counter = 0; counter < bullets.length; counter++) {
	if (modes[counter] == "block") {
		if (testCol(bullets[counter].position.x - (0.5 * Math.sin(bullets[counter].rotation.y))/fps, bullets[counter].position.y + (0.5 * Math.tan(bullets[counter].rotation.x))/fps - 0.5, bullets[counter].position.z - (0.5 * Math.cos(bullets[counter].rotation.y))/fps)) {
    		bullets[counter].position.x -= (0.5 * Math.sin(bullrot[counter].y))/fps;
    		bullets[counter].position.y += (0.5 * Math.tan(bullrot[counter].x))/fps;
    		bullets[counter].position.z -= (0.5 * Math.cos(bullrot[counter].y))/fps;
    	}
    	else {
